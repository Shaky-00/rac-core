#!/usr/bin/env python3
"""Build paper v3 overhead figures and tables from ``results/rac_tracebench_v06_overhead*``.

Does not modify RAC core or ``mcp_data``. Does not re-run experiments unless
...
FIG = ROOT / "artifacts" / "figures_paper_v3"
TBL = ROOT / "artifacts" / "tables_paper_v3"
README = FIG / "README_overhead_artifacts.md"

CSV_PATH = RESULTS / "rac_tracebench_v06_overhead.csv"
SUMMARY_PATH = RESULTS / "rac_tracebench_v06_overhead_summary.json"
...
def write_readme(
    *,
    ...
    if gen_a:
        lines.append("- **Figure A** `fig_overhead_breakdown.{pdf,png,svg}` — RAC step-level component distributions (horizontal boxplots, log-scaled x). **Suggested: main text** (core breakdown).")
    if gen_b:
        lines.append("- **Figure B** `fig_step_latency_by_decision.{pdf,png,svg}` — `total_step_ms` by **benign vs violation** (fallback; no step-level decision in CSV). **Suggested: appendix** unless you add a join story.")
    else:
        lines.append("- **Figure B** — not generated (see §6).")
    if gen_c:
        lines.append("- **Figure C** `fig_trace_latency_by_length.{pdf,png,svg}` — `total_trace_ms` by trace length bucket. **Suggested: appendix** (supporting).")
    else:
    ...
    if not gen_b:
        lines.append("- Figure B would require per-step `decision` (ALLOW/BLOCK) in the overhead CSV or a reliable step-level join; neither is available from committed `results/` alone.")
    if not gen_c:
