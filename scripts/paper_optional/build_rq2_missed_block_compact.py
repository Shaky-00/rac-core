#!/usr/bin/env python3
"""Optional helper for compact missed-block figures from TraceBench summary JSON.

Reads ``rac_tracebench_v06_summary.json`` from ``artifacts/results/`` or
``artifacts/expected/summaries/`` (no rerun). Two layouts:
...
Stacked horizontal-bar panels: **six** baselines vs **seven** component ablations.

**Split PDF outputs:** ``fig_baselines.pdf`` and ``fig_ablations.pdf`` use the
same v2 normalization and styling tuned for side-by-side subfigures in a two-column layout.
"""
...
from io_paths import pick_input, repo_root

ROOT = repo_root()
SUMMARY_PATH = pick_input("rac_tracebench_v06_summary.json")
OUT_DIR = ROOT / "artifacts" / "generated" / "figures_rq2"
...
# Display labels (same keys / order as ``ABLATION_KEYS``; avoid leading "-" in the figure).
