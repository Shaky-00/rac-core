#!/usr/bin/env python3
"""Aggregate TraceBench / MCP summary JSON into one overview (no experiments run)."""
...
from io_paths import pick_input, repo_root, try_pick_input

ROOT = repo_root()
GEN = ROOT / "artifacts" / "generated"
OUT_JSON = GEN / "rac_experiment_overview.json"
OUT_MD = GEN / "rac_experiment_overview.md"
...
def main() -> int:
    ...
    try:
        paths = {
            "tracebench": pick_input("rac_tracebench_v06_summary.json"),
            "overhead": try_pick_input("rac_tracebench_v06_overhead_summary.json"),
            "mcp_fs": try_pick_input("mcp_real_filesystem_v06_summary.json"),
            "planner": try_pick_input("mcp_real_filesystem_planner_v06_summary.json"),
        }
    except FileNotFoundError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 1

    tb = _load(paths["tracebench"])
    oh = _load(paths["overhead"]) if paths["overhead"] else None
    mcp = _load(paths["mcp_fs"]) if paths["mcp_fs"] else None
    planner = _load(paths["planner"]) if paths["planner"] else None
    ...
    if tb is None:
        print(f"ERROR: missing required {paths['tracebench']}", file=sys.stderr)
        return 1
    ...
    overview = {
        ...
        "inputs": {
            ...
            if v is not None and v.is_file()
        },
    ...
    lines = [
        ...
        "",
        f"Generated: `{overview['overview_generated_at']}` (aggregates TraceBench/MCP summary JSON from artifacts paths).",
        "",
    ...
    if real_fs:
        ...
        lines.append(f"*Missing `mcp_real_filesystem_v06_summary.json` (not under artifacts/results or expected).*\n")
    ...
    if planner_section:
        ...
        lines.append(f"*Missing `mcp_real_filesystem_planner_v06_summary.json`.*\n")
