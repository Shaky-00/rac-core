#!/usr/bin/env python3
"""Optional helper for regenerating v2 figures and tables from artifact summaries (no RAC core edits).

Reads ``artifacts/results/`` or ``artifacts/expected/**`` JSON/CSV only. Optionally re-runs the
...
FIG_V2 = ROOT / "artifacts" / "generated" / "figures_v2"
TBL_V2 = ROOT / "artifacts" / "generated" / "tables_v2"
README_FIG = FIG_V2 / "README_figures.md"

INPUTS = {
    "tracebench_summary": R / "rac_tracebench_v06_summary.json",
    "overhead_summary": R / "rac_tracebench_v06_overhead_summary.json",
    "overhead_csv": R / "rac_tracebench_v06_overhead.csv",
    "planner_summary": R / "mcp_real_filesystem_planner_v06_summary.json",
    "planner_results": R / "mcp_real_filesystem_planner_v06_results.csv",
    "overview_json": R / "rac_experiment_overview.json",
}
...
def write_readme(overhead_plot_kind: str, overhead_rerun: bool) -> None:
    rerun_note = "Overhead CSV was regenerated via `scripts/run_rac_tracebench_overhead.py`." if overhead_rerun else "No overhead rerun; existing `artifacts/results/rac_tracebench_v06_overhead.csv` (or `artifacts/expected/data/`) was used."

    text = f"""# Optional figures v2 (`figures_paper_v2`)

Monochrome-friendly figures derived from committed summaries when present. Overhead plots reflect instrumented replay; interpret totals with the overhead table notes.

{rerun_note}

## Figure A — `fig_missed_block_rate.*`

- **Data:** `artifacts/results/rac_tracebench_v06_summary.json` or `artifacts/expected/summaries/rac_tracebench_v06_summary.json` → `false_negative_by_variant` (missed-block rate).
...
- **Data:** `artifacts/results/rac_tracebench_v06_overhead.csv`, summary fields in `artifacts/results/rac_tracebench_v06_overhead_summary.json` (or under `artifacts/expected/` when mirrored locally).
...
- **Data:** `artifacts/results/mcp_real_filesystem_planner_v06_summary.json` or `artifacts/expected/summaries/mcp_real_filesystem_planner_v06_summary.json` (aggregated prevented/materialized counts).
...
| File | Source |
|------|--------|
| `table_variant_missed_block.md` | TraceBench summary |
| `table_overhead_distribution.md` | overhead CSV + summary JSON |
| `table_tracebench_coverage_compact.md` | TraceBench summary `per_family_results` |
| `table_experiment_overview_compact.md` | curated from overview + summaries |
| `table_real_mcp_write_effects.md` | planner results CSV (filtered `write_file` S02) |

## Notes

- No multi-panel “overview” figure is generated in this pack.
- No single figure mixes step percentiles and mean trace latency in one bar chart.
"""
    README_FIG.write_text(text, encoding="utf-8")


def main() -> None:
    p = argparse.ArgumentParser(description="Build v2 figures/tables under artifacts/generated/figures_v2 and tables_v2")
    p.add_argument(
