#!/usr/bin/env python3
"""Optional helper for regenerating derived figures and tables from artifact summaries (no experiments)."""
...
ART_FIG = ROOT / "artifacts" / "generated" / "figures"
ART_TBL = ROOT / "artifacts" / "generated" / "tables"
README = ROOT / "artifacts" / "generated" / "README_experiment_artifacts.md"

INPUTS = {
    "tracebench_summary": R / "rac_tracebench_v06_summary.json",
    "tracebench_results": R / "rac_tracebench_v06_results.csv",
    "overhead_summary": R / "rac_tracebench_v06_overhead_summary.json",
    "mcp_fs_summary": R / "mcp_real_filesystem_v06_summary.json",
    "mcp_fs_results": R / "mcp_real_filesystem_v06_results.csv",
    ...
    "overview_md": R / "rac_experiment_overview.md",
}
...
LABEL_ABBREV = {
    ...
    "HistoryAware": "HIST_SC",
    "Static+History": "STAT+HIST",
...
def table_tracebench_coverage(tb: dict) -> None:
    ...
    lines.extend(
        [
            "",
            "*Derived from `rac_tracebench_v06_summary.json` → `per_family_results` keys. "
            "See `docs/tracebench_result_notes.md` for field definitions and comparison guidance.*",
        ]
    )
...
def table_experiment_overview(ov: dict | None) -> None:
    ...
    lines = [
        ...
        "",
        "*Source: `artifacts/results/rac_experiment_overview.json` or `artifacts/expected/summaries/rac_experiment_overview.json`.*",
    ]
    (ART_TBL / "table_experiment_overview.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_readme(missing: list[str]) -> None:
    lines = [
        ...
        "",
        "Generated by `scripts/paper_optional/build_rac_paper_artifacts.py` from `artifacts/results/` or `artifacts/expected/`.",
        "Regenerate: `python3 scripts/paper_optional/build_rac_paper_artifacts.py`",
        "",
        "Requires **matplotlib** (`pip install 'matplotlib>=3.8,<4'` or OS package `python3-matplotlib`).",
        "",
        "## Figures (`artifacts/generated/figures/`)",
        "",
        ...
        "",
        "## Expected inputs under `artifacts/results/` or `artifacts/expected/`",
        "",
